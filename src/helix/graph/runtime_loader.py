...
from helix.config.settings import HelixSettings
from helix.db import Neo4jFullGraphStore, Neo4jHealthyGraphStore, QdrantGraphIndex
from helix.graph.packaged import JsonlPackagedDemoGraphStoreLoader
from helix.schemas import GraphProfile, HelixBaseModel
...
class LoadedGraphRuntime(HelixBaseModel):
...
def _require_neo4j_settings(settings: HelixSettings) -> None:
    missing = []
...
def _load_qdrant_index(settings: HelixSettings) -> QdrantGraphIndex | None:
