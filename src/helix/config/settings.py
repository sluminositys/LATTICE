...
class HelixSettings(StrictModel):
    app_name: str = "HELIX"
    environment: str = "dev"
...
def load_settings(
    ...
    *,
    ...
    """Load HELIX settings from YAML files plus `HELIX_` environment overrides."""

    config_path = Path(config_dir)
    env_name = environment or os.getenv("HELIX_ENV") or "dev"
    files = [
        "base.yaml",
        "logging.yaml",
        "permissions.yaml",
    ...
    return HelixSettings.model_validate(raw)
...
def _env_overrides() -> dict[str, Any]:
    ...
    for key, value in os.environ.items():
        if not key.startswith(prefix) or key == "HELIX_ENV":
            continue
