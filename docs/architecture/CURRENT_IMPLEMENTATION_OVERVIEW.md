...
`plan-only` 用于只规划和解释阻断原因。`execution` 会先把 G1 投影出来的 G2 runtime graph 当作参考路径；如果 G2 没有可用 workflow path 或 ToolCallSpec，可以通过 `RuntimeCapabilityDiscoverer` 接入运行时自主发现，生成临时 `AgenticExecutionPlan` 和候选图更新，而不是把 G2 当作必须执行的固定流程。
...
- `plan_only` 模式不允许工具执行
- 没有不安全 fallback
- 运行时发现得到的临时执行计划仍然必须通过 `PermissionGate` 和 `RuntimeBackend`
...
demoL0/demoL1 走 packaged JSONL 图资产加载边界；production profile 可以接 Neo4j-backed G0/G1 store。G0 接受审计后的 GraphPatch；G1 通过 `MemoryHealthCompiler` 对已写入 G0 的补丁做 materialization，不作为固定 runtime 流程的来源。
...
- 普通 GraphPatch 写入目标是 G0
- G1 GraphPatch 只用于 `MemoryHealthCompiler` 的健康图 materialization
- 必须有关联 source event
...
候选工具 / workflow 不能直接创建成 active 状态。运行时自主发现可以为当前任务返回临时执行 plan 和临时 ToolCallSpec；任务完成后，实际用到的 workflow/tool/implementation profile 以候选或 probationary GraphPatch 进入 G0，并由健康化流程同步到 G1。
